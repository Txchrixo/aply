...
const OUTCOME_CLS: Record<string, string> = {
  ...
  no_answer: "bg-[#79695E]/12 text-[#79695E] border-[#79695E]/30",
  manual: "bg-[#FFE4B5] text-[#79695E] border-[#CFC5BE]",
};
    ...
    >
      <div className="mx-auto w-full max-w-7xl">
        <div className="flex flex-col gap-2">
          <span className="text-xs font-semibold uppercase tracking-[0.16em] text-[#C65D00]">
            {t("training.eyebrow")}
          </span>
          <h2
            id="training-heading"
          ...
          <p className="max-w-2xl text-base text-[#79695E]">
            {t("training.subtitle")}
          </p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              {loading ? (
                <div className="space-y-3">
                  {Array.from({ length: 4 }).map((_, i) => (
                    <Skeleton
                      key={i}
                      className="h-24 rounded-lg border border-[#CFC5BE] bg-[#FFE4B5]"
                    />
                  ))}
                </div>
              ) : items.length === 0 ? (
                <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-[#CFC5BE] bg-[#FFE4B5] px-6 py-10 text-center">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#FFF4DC] text-[#C65D00]">
                ...
                <ul className="space-y-3 pr-2">
                  {items.map((item) => (
                    <li
                      key={item.id}
                      className="rounded-lg border border-[#CFC5BE] bg-[#FFE4B5] p-3"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-medium text-[#4A2F1A]">
                            {item.jobTitle}
                          </p>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <Button
                            ...
                            className="h-7 w-7 text-[#79695E] hover:bg-[#FFF4DC] hover:text-[#B23A1E]"
                          >
                            <Icon name="x" size={12} />
                          </Button>
                        </div>
                      </div>
                      <p className="mt-2 text-xs leading-relaxed text-[#79695E]">
          ...
          <Card className="gap-4 rounded-xl border-[#CFC5BE] bg-[#FFF4DC] p-5">
            <div className="flex items-center gap-2">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#C65D00] text-[#FFE4B5]">
